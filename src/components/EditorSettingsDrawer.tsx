...
export default function EditorSettingsDrawer({ settings, layout, onChange, plotNote, onPlotNoteChange, onOpenHelp, selectedPageNumbers, onClose }: {
  settings: PageSettings;
  layout: PageLayout;
  onChange: (settings: PageSettings) => void;
  plotNote: string;
  onPlotNoteChange: (value: string) => void;
  onOpenHelp: () => void;
    ...
    <div className="fixed inset-0 z-[75] bg-black/35" onMouseDown={onClose}>
      <aside aria-label="設定" className="ml-auto flex h-[100dvh] w-full max-w-xl flex-col overflow-hidden bg-base shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <PageSettingsPanel settings={settings} layout={layout} onChange={onChange} plotNote={plotNote} onPlotNoteChange={onPlotNoteChange} onOpenHelp={onOpenHelp} selectedPageNumbers={selectedPageNumbers} mobileSurface />
        </div>
      </aside>
    </div>
  );
