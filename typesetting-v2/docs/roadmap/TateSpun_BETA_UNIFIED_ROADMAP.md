...
**Not done in this loop (by instruction):** no push, no deploy. Implementation committed locally as one atomic commit (`a1a00ff` on `fix/fq04-keyboard-compact-layout-2026-09-15`, based on `origin/master` `6dd0860`).

**NEXT (single action):** get this commit reviewed and, once approved, pushed/merged/deployed — then Human repeats the real-device Production check with `?viewportDebug=1` on the real project that failed before: keyboard closed (confirm normal Editor unchanged — Header/title/secondary row all visible), keyboard open (confirm `vv`/shell still ≈435, Header/title/secondary row now hidden, textarea top moves substantially upward, textarea height grows substantially past ~91px, typing several lines feels practical, caret movement/scroll works), then keyboard close (confirm all three rows return exactly, with Settings/Options/Memo/Help still functioning). Do not mark FQ-04 CLOSED/PASS until that Human confirms actual usable layout — the numeric estimate above is not a substitute for that check.
