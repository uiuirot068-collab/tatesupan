...
**NEXT (single action):** Human repeats the real-device Production check once more with `?viewportDebug=1` and reports the full "style mutations (last 10)" log alongside the usual panel contents. If the log shows exactly ONE mutation per keyboard transition, with its `next` value matching `expectedShellHeight`/`visibleHeight` at that time, and the panel's own `own-shell inline style.height`/`computed height` (now kept fresh by the observer) also then reads correctly -- that confirms the effect-ordering hypothesis (no product bug; the prior "stuck" readings were a diagnostic artifact) and closes this thread without a code fix. If instead the log shows two or more mutations per transition (e.g. a write to 434 followed by a write back to 801), that would be the first direct evidence of a genuine second writer and the actual next blocker to chase. Human visual usability (can the human actually type comfortably with the keyboard open) remains authoritative regardless of the numeric result.
