...
**FQ-04 status: ~~PRODUCTION PASS~~ → ROOT CAUSE CONFIRMED / FIX IMPLEMENTED / PRODUCTION HUMAN QA PENDING.** §21's "Production bundle confirmed to contain the fix" claim was correct about the bundle but wrong about the runtime result — a later real-device Production check (`?viewportDebug=1`, gated diagnostic panel added on `de4b7b9`) proved the shell did not actually shrink with the keyboard.
...
**Not done in this loop (by instruction):** no push, no deploy, no roadmap-history rewrite of §21 beyond the one inline strikethrough/supersede note above.

**NEXT (single action):** get this commit (`92ec797` on `fix/tsp-fq04-shell-height-2026-09-15`, based on current `origin/master` `de4b7b9`) reviewed and, once approved, merged/pushed and deployed via the existing Cloudflare Pages Git-integration path — then repeat the exact same `?viewportDebug=1` Production diagnostic that found this bug, expecting `hook visibleHeight` and `shell inline style.height` to now match (both ≈435 while the keyboard is open) instead of the shell staying pinned at ≈801.5.
