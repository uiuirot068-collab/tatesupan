...
**NEXT (single action):** Human repeats the same real-device Production check one more time (keyboard closed, then open) using `?viewportDebug=1` on the real project that failed before. With only one hook instance now possible, `parent hook visibleHeight` and `shell inline style.height` **must** agree by construction (same render, same value). If the shell is STILL wrong at that point, the panel will show `parent hook visibleHeight` *also* wrong (e.g. still ≈801 while the keyboard is open) -- proving the problem is upstream of the diagnostic entirely (the parent's own resize listener not firing/updating on that device), not a display-layer artifact. That result is the single most useful piece of evidence still missing before any product-behavior fix can be justified. Do not mark FQ-04 CLOSED/PASS regardless of outcome until a Human confirms the textarea is actually usable with the keyboard open.
