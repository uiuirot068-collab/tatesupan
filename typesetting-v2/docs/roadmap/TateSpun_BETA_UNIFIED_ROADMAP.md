...
**FQ-05 (Guide sample double-indent) — OPEN → IMPLEMENTED / HUMAN QA PENDING.** Root cause: the guide's `試し読み（宮沢賢治『ポラーノの広場』より）` excerpt (`src/constants/sampleData.ts`, `SAMPLE_PROJECT.content`) had its 4 paragraphs seeded with a leading half-width space (U+0020) instead of the full-width ideographic space (U+3000) every other sample paragraph in the codebase uses (confirmed against the sibling `src/constants/demoData.ts`, which already uses U+3000 correctly). The shared auto-indent exemption (`paragraphNeedsAutoIndent`/`AUTO_INDENT_CHAR` in `src/lib/tategaki.ts`, and the equivalent private `needsAutoIndent`/`AUTO_INDENT_CHAR` in `typesetting-v2/core/compose/line.ts`) only recognizes U+3000 as "already indented"; a leading U+0020 is not exempted, so the renderer's own auto-indent slot stacked on top of the manuscript's own leading space, producing a guide-only 2-cell indent. This is content-only, guide-specific data — no normal new project is seeded with a leading half-width space, so ordinary projects were never affected. **Fix:** corrected the 4 leading characters in `sampleData.ts` from U+0020 to U+3000. The shared renderer/indent logic in `tategaki.ts` and `typesetting-v2/core/compose/line.ts` was deliberately left untouched (existing `typesetting-v2/core/compose/paragraphSemantics.test.ts` already confirms the U+3000 case is correct for normal projects).
**Known latent gap, not fixed here (recorded for future reference, not a blocker):** the auto-indent exemption in both `tategaki.ts` and `line.ts` does not treat a leading half-width space (U+0020) as "already indented," unlike `src/lib/txtTransfer.ts`'s `serializeReadableTxt` (`/^[\s　]/`), which already does. Any normal user manuscript that manually types a leading half-width space would hit the same double-indent. Out of scope for this guide-only fix; flag for a future general-renderer pass if it recurs on real user content.

**FQ-06 (Guide Polano quote flattened ruby) — OPEN → IMPLEMENTED / HUMAN QA PENDING.** Audited the full quoted excerpt in `sampleData.ts` against the reported pattern (kanji immediately followed by its own hiragana reading with no separator). Found exactly two flattened ruby occurrences, both restored to TateSpun's `｜base《reading》` notation:
- `俸給ほうきゅう` → `｜俸給《ほうきゅう》`
...
**FQ-05 status: IMPLEMENTED / HUMAN QA PENDING.**
**FQ-06 status: IMPLEMENTED / HUMAN QA PENDING.**

**NEXT (single action):** Human QA on the Guide only (local dev build, not deployed): confirm the Polano excerpt's paragraph indentation now visually matches an ordinary new project, confirm typing/removing a leading half/full-width space in the Guide no longer produces a Guide-only double-indent, confirm `俸給` renders with `ほうきゅう` as ruby (not duplicated plain text) and `拵` similarly with `こしら` as ruby followed by plain `え直す`, and spot-check the rest of the quoted excerpt for any remaining ruby issues. Also reconfirm ordinary new-project paragraph indentation is unaffected. **PUSH: NO. DEPLOY: NO. DB MUTATION: NO** — this commit exists only on the local branch `fix/tsp-guide-sample-content-2026-09-16`, not pushed to `origin`.
