...
**NEXT (single action):** Human repeats the same real-device Production check (keyboard closed, then open, on the real project that failed before) with `?viewportDebug=1`, and reports the full panel contents — especially the `[data-editor-shell] count`, the three instance-id lines, and the own-shell-vs-global-selector height comparison. That result determines which of Cases A–D applies and is the prerequisite for any product-behavior fix. Do not mark FQ-04 CLOSED/PASS regardless of outcome until a Human confirms the textarea is actually usable with the keyboard open.
