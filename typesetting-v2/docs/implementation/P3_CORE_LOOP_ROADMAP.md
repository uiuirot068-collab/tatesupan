...
- **Status (2026-09-06): AUTOMATED WORK CLOSED (P3-L15 + P3-L15A gap closure). HUMAN GATE G1: PENDING.** `core/layout/assemble.ts`'s `composeCanonicalDocument()` and `core/index.ts` implemented; F19/F20 (verbatim from `REGRESSION_CORPUS_SPEC.md` §1) and an 8-preset sweep (real production `charsPerLine`/`linesPerColumn`/`fontSizePt` from `src/constants/paperSizes.ts`, read-only) all pass. The "minimal local diagnostic viewer" deliverable was produced as two static evidence documents (`typesetting-v2/qa/human/P3_G1_CANONICAL_CORE_REVIEW.md`, `typesetting-v2/qa/evidence/P3_G1_CANONICAL_CORE_EVIDENCE.md`) rather than an interactive tool, given this Loop's timebox. **P3-L15A follow-up:** closed the ImageUnit zero-cost gap this Loop's own regression suite discovered and disclosed (images now consume real canonical extent via `MeasurementFacts`, `FULL` placement forces isolation by reusing the existing `MANUAL_FORCED` mechanism, unresolved intrinsic size produces a structured HOLD) and precisely classified F06 Hanging as Category B (Expected Deferred / Later Logical Stage, per this document's own line 110 — not a G1 blocker). 174/174 tests pass. See `research/PHASE3_LOOP_LOG.md`'s P3-L15 and P3-L15A entries for full evidence. **No Human answer has been filled in; Master is unmodified; Phase 3 is not closed.**
