...
## 11. Revision 3 (Human QA round 2 = core PASS, Review Dock = CHANGES REQUESTED, 2026-09-22)
Round 2's independent A/B/C model, tints and single-click reason were judged correct (the B/C tint difference explicitly accepted as a beta limitation, not to be re-tuned this round). The Review Dock card's *placement* -- permanently mounted below the manuscript -- was judged too tall/cramped, the same verdict as B4's.

- **Rail surface** (desktop, `<main>` width ≥ 1100px, shared architecture -- see `B4_IMPLEMENTATION_RESULT.md` §10 for the full mechanism): `DescriptionCheckDockCard` is unchanged and is now portalled into `ReviewRail` instead of a below-the-manuscript dock. Two pinned cards (B4 + B5) stack **vertically** inside the Rail (it is a narrow sidebar, not a full-width dock, so they are never side by side at any width -- a real behavioural difference from Revision 2 that `descriptionCheck.e2e.mjs`'s `coexistWithHeldGhost` now asserts explicitly instead of the old "same top = side by side" check). The manuscript's height is unaffected by how many tools are pinned on this surface.
- **Compact surface** (phone + narrower desktop, `<main>` width < 1100px): no permanently-mounted card. `DescriptionCheckFooterPill` (a `描写・修飾 N件` count pill, unchanged component, already existed for the mobile collapsed footer) is now the *only* representation while pinned; a tap opens 見直し, now a Bottom Sheet on this surface, which still owns the full candidate list and A/B/C settings.
- **Review Hub readability**: shared change with B4 -- see `B4_IMPLEMENTATION_RESULT.md` §10.
- Tests: `dockViewport` (the rich card interaction test) now runs only at the rail viewport (1280); a new `compactDockViewport` covers the compact surface (390/770) -- no permanent card, the pill opens the Hub, the full list is reachable from inside the sheet. `reviewLayout.e2e.mjs` covers the surface-level mechanics shared with B4 (max-two-pins, Bottom Sheet geometry, no horizontal overflow) once for both tools.
- **A testing-harness gap found and fixed, not a product bug**: see `B4_IMPLEMENTATION_RESULT.md` §10's "real testing-harness gap" note (`descriptionCheck.e2e.mjs`'s `realClick`, dev-only `<nextjs-portal>` badge). Revision 3's Hub spacing increase is what first pushed a B5 control into the badge's dead zone at 390×844.
- Status unchanged: IMPLEMENTED / AUTOMATED QA PASS / HUMAN_GATE / NOT RELEASED, still with the BETA LIMITATION REVIEW (heuristic analysis).
